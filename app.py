...
# ══════════════════════════════════════════════════════════════════════════════
# SIDEBAR (For Extra Controls & Alerts)
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("### ⚙️ Dashboard Controls")
    theme_choice = st.radio("🌓 Theme Mode", ["Dark", "Light"], 
                            index=0 if st.session_state.theme_mode == "dark" else 1, horizontal=True)
    ...
    period_lbl = st.selectbox("📅 Period", list(period_opts.keys()), index=4)
    ...
    use_lstm = st.toggle("Enable LSTM (slower)", value=False)
    mc_sims  = st.slider("Monte Carlo Paths", 100, 2000, 500, step=100)
    st.markdown("---")
    st.markdown("#### 🔀 Comparison")
    compare_raw = st.text_input("Compare Tickers", "MSFT, GOOGL, TCS.NS, INFY.NS")
    compare_tickers = [t.strip().upper() for t in compare_raw.split(",") if t.strip()]
    st.markdown("---")
    st.markdown("#### 📧 Email Alerts")
    enable_gmail = st.toggle("Enable Gmail Alerts", value=False)
    sender = st.text_input("Your Gmail", placeholder="example@gmail.com")
    app_pwd = st.text_input("Gmail App Password", type="password", help="Get from Google Account > Security")
    receiver = st.text_input("Receiver Email", placeholder="notify-me@gmail.com")
    
    st.markdown("---")
    rf_rate = st.number_input("Risk-Free Rate (%)", 0.0, 15.0, 5.0, 0.25) / 100
...
with tabs[2]:
    ...
    if use_lstm:
        ...
        scaled = scaler.fit_transform(close_arr.reshape(-1,1)).flatten()
        lstm_model, X_te_l, y_te_l, y_pred_l, lb = mdl.train_lstm(scaled, look_back=60, epochs=25)
        if lstm_model is not None:
            lstm_fc = mdl.forecast_lstm(lstm_model, scaled, scaler, horizon, lb)
            ...
            y_te_prices = scaler.inverse_transform(y_te_l.reshape(-1,1)).flatten()
            y_pr_prices = scaler.inverse_transform(y_pred_l.reshape(-1,1)).flatten()
            metrics["LSTM"]    = mdl.evaluate(y_te_prices, y_pr_prices)
        else:
    ...
    for mname, met in metrics.items():
        rows.append({
            "Model": mname,
            f"MAE ({c_sym})":   f"{met['MAE']:.3f}",
            f"RMSE ({c_sym})":  f"{met['RMSE']:.3f}",
            "R²":        f"{met['R²']:.4f}",
            "MAPE (%)":  f"{met['MAPE %']:.2f}",
...
with tabs[6]:
    ...
    for col, lbl, val, clr in [(mc1, "Strategy Return", f"{bt_ma['strat_total']*100:+.1f}%", pct_color(bt_ma['strat_total'])),
        ...
        with col:
            st.markdown(f"""<div class="metric-card" style="text-align:center"><div class="m-label">{lbl}</div><div style="font-size:1.1rem;font-weight:600;color:{clr}">{val}</div></div>""", unsafe_allow_html=True)
...
with tabs[7]:
    ...
    if not news_list:
        ...
        for col, lbl, cnt, clr in [(sn1,"Bullish Articles",bull,C["teal"]),
            ...
            with col:
                st.markdown(f"""<div class="metric-card" style="text-align:center"><div class="m-label">{lbl}</div><div style="font-size:2rem;font-weight:700;color:{clr}">{cnt}</div></div>""", unsafe_allow_html=True)

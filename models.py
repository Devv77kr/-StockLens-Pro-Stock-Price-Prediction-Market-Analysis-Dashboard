...
def build_features(close: np.ndarray, lags: int = 30) -> tuple:
    ...
    Features: lag prices, returns, rolling stats, RSI proxy, MACD proxy.
    """
    s = pd.Series(close)
    df = pd.DataFrame()
    ...
    for w in [5, 10, 20, 30]:
        ...
        df[f"min_{w}"] = s.rolling(w).min()
        df[f"max_{w}"] = s.rolling(w).max()
...
def forecast_sklearn(model, close_arr: np.ndarray, n_days: int):
    """Rolling forecast: predict one step, append, repeat."""
...
def _single_feature_row(s: pd.Series) -> np.ndarray:
    """Build the SAME feature vector as build_features for one row."""
    df = pd.DataFrame()
    for lag in [1, 2, 3, 5, 10, 15, 20, 25, 30]:
        df[f"lag_{lag}"] = [s.iloc[-lag] if len(s) > lag else np.nan]
    for n in [1, 3, 5, 10, 20]:
        df[f"ret_{n}"] = [s.pct_change(n).iloc[-1] if len(s) > n else np.nan]
    for w in [5, 10, 20, 30]:
        df[f"ma_{w}"]  = [s.rolling(w).mean().iloc[-1] if len(s) >= w else np.nan]
        df[f"std_{w}"] = [s.rolling(w).std().iloc[-1]  if len(s) >= w else np.nan]
        df[f"min_{w}"] = [s.rolling(w).min().iloc[-1]  if len(s) >= w else np.nan]
        df[f"max_{w}"] = [s.rolling(w).max().iloc[-1]  if len(s) >= w else np.nan]
    delta = s.diff()
    gain  = delta.clip(lower=0).rolling(14).mean()
    loss  = (-delta.clip(upper=0)).rolling(14).mean()
